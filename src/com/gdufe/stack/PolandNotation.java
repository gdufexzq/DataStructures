...
//        Stack<String> s2 = new Stack<>(); // 存储中间结果的栈s2
        ...
        ls.forEach(item -> {
            ...
            if(item.matches("\\d+")) {
                ...
                // 如果是右括号")",则一次弹出s1栈顶元素，并压入s2,直到遇到左括号为止，此时将这一对括号丢弃
                while (!s1.peek().equals("(")) {
                    s2.add(s1.pop());
                }
                s1.pop(); // 弹出(左括号 ,消除小括号
            } else {
                // 当item的优先级小于等于s1栈顶运算符，将s1栈顶的运算符弹出并加入到s2中，再次转到4.1与s1中
                // 新的栈顶运算符相比较
                // 问题：我们缺少一个比较优先级高低的方法
                while (s1.size() != 0 && Operation.getValue(s1.peek()) >= Operation.getValue(item)) {
                    s2.add(s1.pop());
                }
                // 如果s1栈顶元素为(则返回0，小于运算符，循环一样会退出
                // 还需要将item压入栈
                s1.push(item);
            }
        });
...
//        } while (i < s.length());
     ...
     * 3 4 + 5 x 6 -
     * 1)从左到右扫描，将3和4压入堆栈
     * 2)遇到+运算符，因此弹出4和3（4为栈顶元素，3为次顶元素），计算3+4的值，得到7，再将7入栈；
     * 3）将5入栈；
